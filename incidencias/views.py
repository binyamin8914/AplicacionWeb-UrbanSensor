...
def gestion_incidencias(request):
    ...
    if group_name == "SECPLA":
        ...
        incidencias = Incidencia.objects.none() if not departamento else Incidencia.objects.filter(encuesta__departamento=departamento).order_by('-created_at')
    elif group_name == "Direccion":
        direccion = getattr(request.user, 'direccion_encargada', None)
        incidencias = Incidencia.objects.none() if not direccion else Incidencia.objects.filter(encuesta__departamento__direccion=direccion).order_by('-created_at')
    elif group_name == "Cuadrilla":
        cuadrilla = Cuadrilla.objects.filter(encargado=request.user).first()
        incidencias = Incidencia.objects.none() if not cuadrilla else Incidencia.objects.filter(cuadrilla=cuadrilla).order_by('-created_at')
    else:
...
def editar_incidencia(request, incidencia_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if group_name != "Territorial":
                incidencia.cuadrilla = datos.get('cuadrilla')
